  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <el-form-item label="Warehouse">
        <el-select style="width:120px;" v-model="dataForm.wareId" placeholder="Please select warehouse" clearable>
          <el-option :label="w.name" :value="w.id" v-for="w in wareList" :key="w.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="Status">
        <el-select style="width:120px;" v-model="dataForm.status" placeholder="Please select status" clearable>
      ...
      <el-form-item label="Keyword">
        <el-input style="width:120px;" v-model="dataForm.key" placeholder="Parameter Name" clearable></el-input>
      </el-form-item>
      <el-form-item>
        ...
        <el-dropdown @command="handleBatchCommand" :disabled="dataListSelections.length <= 0">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="delete">Batch Delete</el-dropdown-item>
            <el-dropdown-item command="merge">Merge Order</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </el-form-item>
    </el-form>
    ...
    >
      ...
      <el-table-column prop="status" header-align="center" align="center" label="Status">
        <template slot-scope="scope">
          ...
          <el-tag type="danger" v-if="scope.row.status==4">Purchase Failed</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog title="Merge to Order" :visible.sync="mergedialogVisible">
      <!-- id  assignee_id  assignee_name  phone   priority status -->
      <el-select v-model="purchaseId" placeholder="Please select" clearable filterable>
        <el-option
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="mergeItem">Confirm</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    mergeItem() {
      ...
      if (!this.purchaseId) {
        this.$confirm(
          "No purchase order selected, a new order will be created automatically for merging. Confirm?",
          "Tip",
          {
    ...
    handleBatchCommand(cmd) {
      ...
      if (cmd == "merge") {
        if (this.dataListSelections.length != 0) {
          ...
          this.$alert("Please select requirements to merge first", "Tip", {
            confirmButtonText: "Confirm",
            callback: action => {}
          });
        }
